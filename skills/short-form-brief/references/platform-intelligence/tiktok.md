...
- **Verbatim example A:** Billie Eilish stitched a video by @gabrieeelala (a clip of her dog jumping with a long stick), opening with the dog clip and reacting with a teary close-up `[pattern-observed; URL not pinned]` — documented as a 2024 viral Stitch case across creator-recap content; specific Billie Eilish post URL not pinned.
- **Verbatim example B:** @mckenzibrooke × Amazon Prime Video brand collaboration: stitched Prime's TikTok clip with a comedic counter-frame `[pattern-observed; URL not pinned]` — pattern attributed to a 2024 brand-collab Stitch recap; specific post URL not pinned.
...
| Caption character limit | 4,000 characters (raised from 2,200 in 2023; 2,200 was raised from 300 mid-2022) | Practitioner-derived (no platform-doc citation in current sources block — see §7) |
...
| Hashtag norm | 3–5 hyper-specific hashtags (CapCut/ByteDance editor explicit guidance) | Practitioner consensus across multiple secondary sources (specific source IDs not yet pinned to frontmatter — see §7) |
...
| **Hashtag stuffing (>7 tags)** | Sends "mixed signals" to recsys per practitioner consensus + CapCut/ByteDance editor guidance; dilutes niche targeting. | Brief specifies more than 5 hashtags OR mixes broad (#fyp, #foryou, #viral) with niche tags hoping for both. | Practitioner consensus + CapCut creator guidance (specific source IDs not yet pinned — see §7) |
...
- **Caption character limit (4,000 chars) primary-source citation gap.** The 2023 expansion from 2,200 → 4,000 is widely repeated by practitioners (Hootsuite, OpusClip, jera.bean's announcement) but no TikTok Help Center URL has been pinned to the frontmatter sources block. The numeric value is correct in practitioner consensus; the citation chain is not yet primary. To resolve: locate and add the relevant TikTok Help Center / Newsroom announcement.
- **Hashtag-norm citation chain.** The 3–5 hyper-specific hashtag norm and the >7 stuffing penalty are both practitioner consensus across multiple sources, but the inline citations (`accio-hashtag-2025`, `admetrics`, `sproutsocial`) were not registered in the frontmatter sources block at first ship. Re-research pass needs to convert these to proper source entries with URLs + accessed dates, or replace with TikTok primary docs if they exist.
...
| 2026-05-08 | Phase 0.5b URL repair (Bucket A). Findings 1-4 closed: Archetype 1 example B ("POV: you just hit $1M ARR"), Archetype 2 examples A+B (Billie Eilish stitch / @mckenzibrooke × Prime Video brand stitch), and Archetype 4 example A (Codie Sanchez laundromat) — all relabeled `[pattern-observed; URL not pinned]` with practitioner-observed framing preserved. No fabricated URLs. `last_verified` bumped to 2026-05-08. | assistant-2026-05-08 |
