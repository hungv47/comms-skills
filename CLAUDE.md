...
Brand identity, persuasive copy, campaign planning, landing-page architecture, post-launch landing-page evaluation, design briefs, search visibility, humanization, localization polish, outbound, paid Meta ads, short-form video briefs, and platform-native social-copy. 13 skills + orchestrate-marketing.
...
campaign-plan (channel strategy + calendar)
  ...
  ├─ eval-loop (measurable page loop) → lp-eval (post-launch evidence)
...
Landing pages: lp-brief owns construction-time conversion best practices for new pages and redesigns. lp-eval owns post-launch scoring, but only inside an existing eval-loop workspace.

Horizontal: copywriting, humanize, vn-tone — invoked at any stage.

## Recommended Starting Point
Run `icp-research` (from [research-skills](https://github.com/hungv47/research-skills)) first — it creates `research/product-context.md`, the canonical cross-stack record consumed by 13+ downstream skills.

## Artifacts
Pipeline outputs write to `skills-resources/marketing/` unless the work belongs to a measurable eval loop. For campaign/page/content/ad/email initiatives that will be evaluated over cycles, write loop-specific strategy or execution artifacts under `skills-resources/marketing/loops/[slug]/strategy/` or `skills-resources/marketing/loops/[slug]/execution/` following `meta-skills/references/eval-loop-spec.md`. Cross-stack records live in the top-level `research/` and `brand/` folders:
...
- `skills-resources/research/short-form-research/[slug].md` (from short-form-research in research-skills — per-platform best-practice catalog, consumed by short-form-brief)
- `skills-resources/marketing/campaign-plan.md`
- `skills-resources/marketing/content/[slug].copy.md` (from copywriting)
...
- `skills-resources/marketing/content/[slug].vn-tone.md`
- `skills-resources/marketing/cold-outreach/[slug].md` (+ `[slug].rationale.md` + `[slug].critic-score.md`)
- `skills-resources/marketing/ad-copy/[audience-temp]-[date]-[slug].md` (+ `[slug].rationale.md` + `[slug].critic-score.md`) — from ad-copy
...
- `skills-resources/marketing/lp-brief/[slug]/brief.md` (landing-page redesign brief — from lp-brief; rev versions at `v[N]/brief.md`)
- `skills-resources/marketing/lp-brief/[slug]/handoff-*.md` (per-target hand-off prompts — from lp-brief)
- `skills-resources/marketing/lp-brief/[slug]/asset-slots/*.prompt.md` (per-slot generative prompts — written by design-brief when invoked on a slot)
- `skills-resources/marketing/loops/[slug]/evals/[date]-cycle-N.md` (post-launch landing-page evaluation — from lp-eval)
- `skills-resources/marketing/loops/[slug]/results.tsv` (keep/discard/watch/blocked ledger — appended by eval skills)
- `skills-resources/marketing/design-briefs/[slug].md` (per-asset graphic-design brief — from design-brief)
- `skills-resources/marketing/short-form-brief/[slug]/brief.md` (hero short-form video brief — from short-form-brief)
- `skills-resources/marketing/short-form-brief/[slug]/variants/[platform].md` (per-platform variant briefs — from short-form-brief, when multi-platform)
- `skills-resources/marketing/loops/[slug]/strategy/*.md` (loop-local strategy artifacts for measurable initiatives)
- `skills-resources/marketing/loops/[slug]/execution/*.md` (publish-ready marketing/content assets for measurable initiatives; publishing remains external)

## Cross-Stack (Optional)
campaign-plan and lp-brief can read research artifacts for alignment:
- `skills-resources/meta/sketches/prioritize-*.md` (from prioritize in research-skills)
- `skills-resources/meta/records/targets-*.md` (from funnel-planner in research-skills)
`npx skills add hungv47/research-skills`

## Pre-Dispatch Protocol

All 13 skills follow the canonical Pre-Dispatch protocol (`meta-skills/references/pre-dispatch-protocol.md`). Cold Start (3-7 bundled questions, one round-trip) when context is missing; Warm Start (summary + optional probe) when artifacts/experience cover what's needed. Answers persist to `skills-resources/experience/{product,audience,brand,business,goals,content}.md` so subsequent skills never re-ask. Hard-gated skills (`design-brief`, `lp-brief`, `lp-eval`) gate before cold-start questioning — recommend `brand-system` when brand gates fail and `eval-loop` when an evaluation loop is missing. `cold-outreach` has the most-elaborate cold-start (7 questions + Missing-Input Hard Blocks for mode/channel/target/proof); `ad-copy` is a close second (7 questions + audience-temp + creative-format hard-blocks). `brand-system` carries the canonical 13-platform target list as a catalog inside its Pre-Dispatch. `short-form-brief` writes brand_mode + production_mode to `skills-resources/experience/content.md`.
...
**Safety gates supersede `--fast`.** Hard-gated skills enforce gates regardless of `--fast`: `design-brief` and `lp-brief` cannot ship a brief without brand-system context (the gate fires before orchestration); `lp-eval` cannot score without an existing eval loop and primary metric evidence; `ad-copy`'s Meta policy + claim-substantiation format-checker runs regardless of speed (ad-policy compliance is not optional); `cold-outreach`'s Missing-Input Hard Blocks for mode/channel/target/proof still fire. The contract is "skip the heavy lift, not the guardrails."
...
State detection across all marketing skills (especially `orchestrate-marketing`) reads `skills-resources/manifest.json` — a derived index of artifact metadata (producer, date, status, schema version, staleness, summary). The manifest is rebuilt from artifact frontmatter by `meta-skills/scripts/manifest-sync.ts`; skills don't write to it directly. See [`../meta-skills/references/manifest-spec.md`](../meta-skills/references/manifest-spec.md) for the full contract. When writing into `skills-resources/marketing/loops/`, also follow [`../meta-skills/references/eval-loop-spec.md`](../meta-skills/references/eval-loop-spec.md): strategy artifacts use `lifecycle: strategy`; publish-ready marketing/content artifacts use `lifecycle: execution`; evaluation artifacts use `lifecycle: evaluation`. Skills that produce artifacts (brand-system, copywriting, campaign-plan, lp-brief, lp-eval, seo, cold-outreach, ad-copy, design-brief, humanize, vn-tone, short-form-brief, social-copy) must write the required frontmatter fields (`skill`, `version`, `date`, `status`) and call sync as their last step.
...
- `lp-eval` — 4 agents (metric-ingest, diagnosis, recommendation, critic). Layer 1 parallel (metric-ingest + diagnosis) → Layer 2 recommendation → Layer 3 critic. Post-launch landing-page evaluator that writes loop-local `evals/[date]-cycle-N.md`, appends `results.tsv`, and promotes learnings only for high-confidence evidence. Requires an existing eval-loop workspace and primary metric evidence; does not perform generic heuristic audits or produce redesign briefs.
- `short-form-brief` — 9 agents (format, voc-extraction, production-mode, hook, storyboard, audio, copy-pack, platform-tailor, critic). Layer 1 parallel foundation (format + voc-extraction + production-mode) → Layer 1.5 parallel craft (hook + storyboard + audio + copy-pack) → Layer 2 sequential (platform-tailor for variants → critic with 4 sub-critics: hook + production + algorithm-fit + brand-fit). Per-asset video brief consuming the matching `skills-resources/research/short-form-research/[slug].md`. Hard cap: 1 hero + 2 variants per invocation. Brand modes: founder | company. Polish chain (vn-tone | humanize) auto-routes per (market, brand_mode) on spoken-line section.
...
## Recent Changes
- 2026-05-13: Marketing 6.1.0 — added `lp-eval`, a loop-native post-launch landing-page evaluator. It requires an existing `/eval-loop` workspace, real primary metric evidence, and writes `skills-resources/marketing/loops/[slug]/evals/[date]-cycle-N.md` plus a `results.tsv` row. Stack surface: 12 → 13 skills.
