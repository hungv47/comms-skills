...
## Artifacts
Pipeline outputs write to `.agents/skill-artifacts/mkt/` unless the work belongs to a measurable eval loop. For campaign/page/content/ad/email initiatives that will be evaluated over cycles, write loop-specific strategy or execution artifacts under `skills-resources/loops/[slug]/strategy/` or `skills-resources/loops/[slug]/execution/` following `meta-skills/references/eval-loop-spec.md`. Cross-stack records live in the top-level `research/` and `brand/` folders:
...
- `.agents/skill-artifacts/research/short-form-research/[slug].md` (from short-form-research in research-skills — per-platform best-practice catalog, consumed by short-form-brief)
...
- `skills-resources/loops/[slug]/evals/[date]-cycle-N.md` (post-launch landing-page evaluation — from lp-eval)
- `skills-resources/loops/[slug]/results.tsv` (keep/discard/watch/blocked ledger — appended by eval skills)
...
- `skills-resources/loops/[slug]/strategy/*.md` (loop-local strategy artifacts for measurable initiatives)
- `skills-resources/loops/[slug]/execution/*.md` (publish-ready marketing/content assets for measurable initiatives; publishing remains external)
...
All 13 skills follow the canonical Pre-Dispatch protocol (`meta-skills/references/pre-dispatch-protocol.md`). Cold Start (3-7 bundled questions, one round-trip) when context is missing; Warm Start (summary + optional probe) when artifacts/experience cover what's needed. Answers persist to `.agents/experience/{product,audience,brand,business,goals,content}.md` so subsequent skills never re-ask. Hard-gated skills (`design-brief`, `lp-brief`, `lp-eval`) gate before cold-start questioning — recommend `brand-system` when brand gates fail and `eval-loop` when an evaluation loop is missing. `cold-outreach` has the most-elaborate cold-start (7 questions + Missing-Input Hard Blocks for mode/channel/target/proof); `ad-copy` is a close second (7 questions + audience-temp + creative-format hard-blocks). `brand-system` carries the canonical 13-platform target list as a catalog inside its Pre-Dispatch. `short-form-brief` writes brand_mode + production_mode to `.agents/experience/content.md`.
...
## Recent Changes
- 2026-05-13: Marketing 6.1.0 — added `lp-eval`, a loop-native post-launch landing-page evaluator. It requires an existing `/eval-loop` workspace, real primary metric evidence, and writes `skills-resources/loops/[slug]/evals/[date]-cycle-N.md` plus a `results.tsv` row. Stack surface: 12 → 13 skills.
